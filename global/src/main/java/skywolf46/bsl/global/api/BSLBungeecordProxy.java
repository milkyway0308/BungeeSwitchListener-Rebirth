...
//            System.out.println("Writing " + packet.getClass() + " / " + packet.getID());
            int port = BSLCoreAPI.getPort();
            if (port <= 0) {
                System.err.println("Something is wrong; Bukkit port is lower than 1");
                return;
            }
            channel().send(new PacketBroadcast(BSLCoreAPI.getPort(), packet));
        }
    }
